import type { AppScreen } from "@/types/scan";

interface NavBarProps {
  currentScreen: AppScreen;
  hasResults: boolean;
  onNavigate: (screen: AppScreen) => void;
}

/**
 * Modeling Studio is three modules over one model parser. The labels name the
 * modules rather than the screens, so the structure the docs describe is the
 * structure the app shows. History is a utility, not a module.
 */
const navItems: {
  screen: AppScreen;
  label: string;
  description: string;
  iconPath: string;
}[] = [
  {
    screen: "sidekick",
    label: "Sidekick",
    description: "Guided project",
    iconPath:
      "M9 6.75V15m6-6v8.25m.503 3.498l4.875-2.437c.381-.19.622-.58.622-1.006V4.82c0-.836-.88-1.38-1.628-1.006l-3.869 1.934c-.317.159-.69.159-1.006 0L9.503 3.311a1.125 1.125 0 00-1.006 0L3.622 5.748A1.125 1.125 0 003 6.754v10.505c0 .836.88 1.38 1.628 1.006l3.869-1.934c.317-.159.69-.159 1.006 0l4.994 2.497c.317.158.69.158 1.006 0z",
  },
  {
    screen: "drop",
    label: "Analyzer",
    description: "AI readiness",
    iconPath:
      "M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z",
  },
  {
    screen: "results",
    label: "Findings",
    description: "Scan results",
    iconPath:
      "M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75zM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V8.625zM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V4.125z",
  },
  {
    screen: "dataagent",
    label: "Data Agent",
    description: "Agent setup",
    iconPath:
      "M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09zM18.259 8.715L18 9.75l-.259-1.035a3.375 3.375 0 00-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 002.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 002.456 2.456L21.75 6l-1.035.259a3.375 3.375 0 00-2.456 2.456z",
  },
  {
    screen: "history",
    label: "History",
    description: "Change log",
    iconPath: "M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z",
  },
];

export function NavBar({ currentScreen, hasResults, onNavigate }: NavBarProps) {
  return (
    <div className="flex items-stretch gap-2 px-6 pb-4 pt-2 shrink-0">
      {navItems.map(({ screen, label, description, iconPath }) => {
        const enabled = screen === "results" ? hasResults : true;
        const active = currentScreen === screen;
        return (
          <button
            key={screen}
            disabled={!enabled}
            onClick={() => onNavigate(screen)}
            className={`
              flex-1 flex flex-col items-center justify-center gap-1 py-3 rounded-xl border
              transition-all duration-150
              ${active
                ? "bg-blue-600/10 border-blue-500/40 text-blue-400 shadow-lg shadow-blue-500/5"
                : enabled
                  ? "bg-slate-900/80 border-slate-700 text-slate-400 hover:border-slate-500 hover:text-slate-200 hover:bg-slate-800/80"
                  : "bg-slate-900/30 border-slate-800 text-slate-700 cursor-not-allowed"
              }
            `}
          >
            <svg
              width="19"
              height="19"
              className="shrink-0"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={1.5}
            >
              <path strokeLinecap="round" strokeLinejoin="round" d={iconPath} />
            </svg>
            <span className="text-[13px] font-semibold leading-tight">{label}</span>
            <span className={`text-[10px] leading-tight ${active ? "text-blue-500/70" : "text-slate-600"}`}>
              {description}
            </span>
          </button>
        );
      })}
    </div>
  );
}
